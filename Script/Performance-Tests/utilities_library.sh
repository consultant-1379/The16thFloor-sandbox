...
log_files_creation() {
# $1 represents test mode ASYNC.
# If it is ASYNC = false, the 3 log files are prepared..
# If it is ASYNC = true, nothing is performed.

  if [ $1 = false ]; then

	# setup globalLogin file
	if [ -f ${globalLogin} ]; then
			cp ${globalLogin} "${globalLogin}-old"
			rm -f ${globalLogin}
	fi
	echo "Haproxy: $HAPROXY_URL - Use of JWT Cookie: $use_jwt" > ${globalLogin}
	echo "Users: $USERS" >> ${globalLogin}
	echo "Iteration: $ITERATION" >> ${globalLogin}
	echo "Sessions: $SESSIONS" >> ${globalLogin}
	echo "Verifications: $VERIFY_PER_SESSION" >> ${globalLogin} #ITERATION_VALIDATE
	echo "Do verify: $LOGIN" >> ${globalLogin}
	echo "Do verify: $VERIFICATION" >> ${globalLogin}
	echo "Do logout: $LOGOUT" >> ${globalLogin}
	echo "ERROR rate: $TEST_ERROR_THRESHOLD %" >> ${globalLogin}
	echo "" >> ${globalLogin}
	echo "In this file we have minimum, average and maximum vales for LOGIN actions." >> ${globalLogin}
	echo "" >> ${globalLogin}
	echo $headers >> ${globalLogin}
	echo "File: ${globalLogin}"

	# setup globalLogout file
	if [ -f ${globalLogout} ]; then
			cp ${globalLogout} "${globalLogout}-old"
			rm -f ${globalLogout}
	fi
	echo "Haproxy: $HAPROXY_URL" > ${globalLogout}
	echo "Users: $USERS" >> ${globalLogout}
	echo "Iteration: $ITERATION" >> ${globalLogout}
	echo "Sessions: $SESSIONS" >> ${globalLogout}
	echo "Verifications: $VERIFY_PER_SESSION" >> ${globalLogout}
	echo "Do verify: $LOGIN" >> ${globalLogout}
	echo "Do verify: $VERIFICATION" >> ${globalLogout}
	echo "Do logout: $LOGOUT" >> ${globalLogout}
	echo "ERROR rate: $TEST_ERROR_THRESHOLD %" >> ${globalLogout}
	echo "" >> ${globalLogout}
	echo "In this file we have minimum, average and maximum vales for LOGOUT actions." >> ${globalLogout}
	echo "" >> ${globalLogout}
	echo $headers >> ${globalLogout}
	echo "File: ${globalLogout}"

	# setup globalVerify file
	if [ -f ${globalVerify} ]; then
			cp ${globalVerify} "${globalVerify}-old"
			rm -f ${globalVerify}
	fi
	echo "Haproxy: $HAPROXY_URL - Use of JWT Cookie: $use_jwt" > ${globalVerify}
	echo "Users: $USERS" >> ${globalVerify}
	echo "Iteration: $ITERATION" >> ${globalVerify}
	echo "Sessions: $SESSIONS" >> ${globalVerify}
	echo "Verifications: $VERIFY_PER_SESSION" >> ${globalVerify}
	echo "Do verify: $LOGIN" >> ${globalVerify}
	echo "Do verify: $VERIFICATION" >> ${globalVerify}
	echo "Do logout: $LOGOUT" >> ${globalVerify}
	echo "ERROR rate: $TEST_ERROR_THRESHOLD %" >> ${globalVerify}
	echo "" >> ${globalVerify}
	echo "In this file we have minimum, average and maximum vales for VERIFY actions." >> ${globalVerify}
	echo "" >> ${globalVerify}
	echo $headers >> ${globalVerify}
	echo "File: ${globalVerify}"
