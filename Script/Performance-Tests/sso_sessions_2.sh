...
session_configuration_and_verify ${SESSION_TIMEOUT_MINUTE} ${SESSION_TIMEOUT_MAX_DURATION_MINUTE}
...
echo "IDLE SESSION TEST"

echo "===== IDLE Session test to be performed with credentials $usr at `date`" >> ${tmp_logfile}
login ${usr} ${pwd} ${session_cookie} ${tmp_logfile}
verify_token ${session_cookie} ${usr} ${SESSION_ISVALID} ${tmp_logfile}

echo "Sleeping for ${SLEEP_SESSION_TIMEOUT_IDLE}s to test session idle timeout of ${SESSION_TIMEOUT_IDLE}s for user $usr" >> ${tmp_logfile}
sleep ${SLEEP_SESSION_TIMEOUT_IDLE}
verify_token ${session_cookie} ${usr} ${SESSION_ISINVALID} ${tmp_logfile}
echo "" >> ${tmp_logfile}

echo "MAX SESSION TEST"

echo "===== MAX Session test to be performed with credentials $usr at `date`" >> ${tmp_logfile}
login ${usr} ${pwd} ${session_cookie} ${tmp_logfile}
verify_token ${session_cookie} ${usr} ${SESSION_ISVALID} ${tmp_logfile}

COUNTER=1
while [  $COUNTER -le ${ITERATIONS_TO_REACH_MAX_TIMEOUT_EXPIRATION} ]; do
    echo "============== Iteration $COUNTER of ${ITERATIONS_TO_REACH_MAX_TIMEOUT_EXPIRATION} ==============" >> ${tmp_logfile}
    echo "Sleeping for ${SLEEP_LOWER_THAN_SESSION_TIMEOUT_IDLE}s in order to progress through the max timeout of ${SESSION_TIMEOUT_MAX_DURATION}s but keep the session active" >> ${tmp_logfile}
...
echo "Completed after ${COUNTER} iterations===" >> ${tmp_logfile}
finalTimeToWait=$((${SLEEP_SESSION_TIMEOUT_MAX_DURATION} - $COUNTER*${SLEEP_LOWER_THAN_SESSION_TIMEOUT_IDLE}))
echo "It's needed to wait final extra ${finalTimeToWait}s to get the session expired by max time ${SESSION_TIMEOUT_MAX_DURATION}" >> ${tmp_logfile}
...
session_configuration_and_verify ${SESSION_IDLE_DEFAULT} ${SESSION_MAX_DEFAULT}
